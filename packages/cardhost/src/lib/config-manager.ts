...
export class ConfigManager {
   ...
   */
  async loadOrCreate(defaultRouterUrl?: string): Promise<CardHostPersistedConfig> {
    if (this.config) {
      return this.config;
    }
   ...
   */
  private async createNew(routerUrl?: string): Promise<CardHostPersistedConfig> {
  ...
  private validateConfig(config: CardHostPersistedConfig): void {
    ...
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(config.uuid)) {
