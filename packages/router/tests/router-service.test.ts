...
describe("RouterService", () => {
  ...
  describe("Controller Authentication", () => {
    ...
    it("should reject short bearer token", async () => {
      ...
      await expect(router.authenticateController(shortToken)).rejects.toThrow(
    ...
    it("should validate session tokens", async () => {
      const sessionToken =
        await router.authenticateController("valid-bearer-123");
  ...
  describe("Cardhost Authentication", () => {
    it("should complete challenge-response authentication", async () => {
      ...
      const isAuthenticated = await router.verifyCardhostAuth(
        uuid,
        challenge,
        signature,
      );
  ...
  describe("Relay Session Creation", () => {
    it("should create relay session with authenticated parties", async () => {
      ...
      const challenge = await router.initiateCardhostAuth(
        cardhostUuid,
        publicKey,
      );
      const signature = await signChallenge(challenge, privateKey);
      ...
      const relayId = router.createRelaySession(
        sessionToken.token,
        cardhostUuid,
      );
